...
/**
 * Classe de demande utilisateur
 * 
 * @author Yaadanial
 *
 */
public class LCABPrompts {

	private LCABPlugin plugin = null;

	public LCABPrompts(LCABPlugin plugin) {
		this.plugin = plugin;
	}
	...
	private class TeamColorPrompt extends ValidatingPrompt {
		...
		protected Prompt acceptValidatedInput(ConversationContext context, String input) {
			...
			plugin.createTeam((String) context.getSessionData("nomTeam"), (ChatColor) context.getSessionData("color"));
		...
		protected boolean isInputValid(ConversationContext context, String input) {
			for (String string : colors) {
				if (ChatColor.stripColor(string).equalsIgnoreCase(input))
					return true;
			}
			return false;
		}
	...
	private enum StringToChatColor {
		...
		public static ChatColor getChatColorByName(String name) {
			for (StringToChatColor stcc : values()) {
				if (stcc.name.equalsIgnoreCase(name))
					return stcc.color;
			}
			return null;
		}
	...
	public PlayerPrompt getPP() {
		return new PlayerPrompt(plugin);
	}
}
